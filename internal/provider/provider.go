...
import (
	...
	"github.com/nokia/eda/apps/terraform-provider-aaa/internal/eda/utils"
...
var _ provider.Provider = (*aaaProvider)(nil)

func New(ver string) func() provider.Provider {
	return func() provider.Provider {
		return &aaaProvider{version: ver}
	}
}

type aaaProvider struct {
	version string
}
...
func (p *aaaProvider) Schema(ctx context.Context, req provider.SchemaRequest, resp *provider.SchemaResponse) {
...
func (p *aaaProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
...
func (p *aaaProvider) Metadata(ctx context.Context, req provider.MetadataRequest, resp *provider.MetadataResponse) {
	resp.TypeName = "aaa-v1"
	resp.Version = p.version
}

func (p *aaaProvider) DataSources(ctx context.Context) []func() datasource.DataSource {
	return []func() datasource.DataSource{
		NewAppGroupDataSource,
		NewAuthenticationPolicyDataSource,
		NewAuthenticationPolicyListDataSource,
		NewAuthenticationPolicyStateDataSource,
		NewAuthenticationPolicyStateListDataSource,
		NewNodeGroupDataSource,
		NewNodeGroupListDataSource,
		NewResourceListDataSource,
		NewServerGroupDataSource,
		NewServerGroupListDataSource,
		NewServerGroupStateDataSource,
		NewServerGroupStateListDataSource,
	}
}

func (p *aaaProvider) Resources(ctx context.Context) []func() resource.Resource {
	return []func() resource.Resource{
		NewAuthenticationPolicyResource,
		NewAuthenticationPolicyStateResource,
		NewNodeGroupResource,
		NewServerGroupResource,
		NewServerGroupStateResource,
	}
}
